...
async function main() {
  ...
  const urls: Array<{ path: string; changefreq: string; priority: string }> = [
    ...
    ...subjectCodes.map((code) => ({ path: `/subject/${encodeURIComponent(code)}`, changefreq: "weekly", priority: "0.8" })),
    ...courseCodes.map((code) => ({ path: `/course/${encodeURIComponent(code)}`, changefreq: "weekly", priority: "0.7" })),
    ...professorIds.map((id) => ({ path: `/professor/${encodeURIComponent(id)}`, changefreq: "weekly", priority: "0.6" })),
  ];
  ...
  await Promise.all([writeFile(join(PUBLIC_DIR, "sitemap.xml"), xml, "utf8"), writeFile(join(PUBLIC_DIR, "robots.txt"), robotsText, "utf8")]);
